# {heading(Пример расчёта характеристик инсталляции)[id=installation_characteristics_calculation]}
...
### {heading(Расчёты количества ВУ)[id=installation_calculations_number_compute_nodes]}
...
#### {heading(Расчёт количества ВУ с высокопроизводительными процессорами)[id=installation_calculations_number_compute_nodes_processors]}
...
#### {heading(Расчёт количества ВУ с коэффициентом переподписки CPU 2)[id=installation_calculations_number_compute_nodes_cpu]}
...
#### {heading(Расчёт параметров одного сервера)[id=installation_calculations_for_ceph_1]}

1. Для серверов Ceph выбрано шасси с 8 дисками SSD NVMe объёмом 7.68 ТБ, пропускная способность диска — 2500 МБайт/сек.

2. Количество OSD на 1 NVMe диск — 2, количество OSD на сервер: 

   {formula(Q_{sOSD} = {2 \ast 8} = 16)}

3. Пропускная способность массива дисков одного сервера (Мбайт/сек): 
...
4. Требуемое количество ядер CPU на сервер: 
...
5. Требуемый объём оперативной памяти (ГБ): 

   {formula(V_{RAM} = {16 * 4 + 5 + 2 + 4} = 75)}

   Наиболее подходящим объёмом памяти под рассчитанные требования является 128 ГБ.

6. Для четырёх сетевых карт с пропускной способностью 25 Гбит/сек, пропускная способность сети Ceph Internal: 

   {formula(T_{int} \approx 0.85 * \frac {4 \ast 25 \ast 10^9}{8 \ast 2^{20}} = 10133)}
    
   Полученное значение отличается от пропускной способности «на чтение» приблизительно на 20%. Можно допустить, что данное отличие будет оказывать влияние только в моменты пиковой нагрузки.
...
#### {heading(Расчёт количества серверов и кластеров)[id=installation_calculations_for_ceph_servers]}
...
Ёмкость кластера из 9 узлов (TiB):

{formula(V_{cluster} = 0.7 * \frac {8 \ast 6.99 \ast 9}{3} \approx 117)} 

Ёмкость кластера из 3 узлов (TiB):
...
1. Для серверов High-IOPS Storage выбрано шасси с 8 дисками SSD NVMe объёмом 7,68 ТБ, пропускная способность диска — 2500 МБайт/сек.
...
1. Для четырёх сетевых карт с пропускной способностью 25 Гбит/сек, пропускная способность сети:
...
## {heading(Расчёты количества серверов слоя прикладных сервисов)[id=installation_calculations_application_service_layer_servers]}

В каждой зоне доступности располагается по одному узлу evh с характеристиками, приведёнными в {linkto(#tab_application_service_layer_servers)[text=таблице %number]}.
...
* Контроллер 1: HAProxy, Nova, Cinder, Neutron и иные компоненты, не перечисленные в данном списке и не размещаемые в служебном кластере Kubernetes.
* Контроллер 2: etcd и служебные БД.
...
{caption(Таблица {counter(table)[id=numb_tab_final_requirements_for_data_center_port_capacity2]} — Характеристики оборудования для сетевой фабрики)[align=right;position=above;id=tab_final_requirements_for_data_center_port_capacity2;number={const(numb_tab_final_requirements_for_data_center_port_capacity2)}]}
[cols="1,1,1,1,1"]
|===
