...
![Методы разделения {var(sys2)}](./assets/design_separation_method_zone_region.png){params[width=80%]}
{/caption}
...
В любой инсталляции Платформы существует как минимум одна зона доступности.

</warn>

Зоны доступности должны быть связаны между собой высокоскоростными каналами связи с низким уровнем задержек. По сравнению с требованиями к каналам связи между регионами минимальные требования связи между зонами доступности более высокие — не менее 10 Гбит/с при задержках не более 5 мс.
...
![Размещение приложения, требующих кворума сервисов, в трёх зонах доступности](./assets/design_separation_method_zone_region_1_3.png){params[width=80%]}
{/caption}
...
В отличие от зон доступности механизм агрегатов недоступен конечному пользователю, размещение ВМ в определённом агрегате выполняется автоматически на основании метаданных, которые задаются администраторами в свойствах типа ВМ.
...
![Расположение всех управляющих узлов в одном домене отказа](./assets/design_separation_method_region_msk.png){params[width=55%]}
{/caption}
...
![Архитектура с управляющим слоем, «растянутым» по трём доменам отказа](./assets/design_separation_method_region_msk_1_3.png){params[width=65%]}
{/caption}
