...
## {heading(Сетевые узлы)[id=calculations_network_nodes]}
...
где {formula(N)} — требуемое количество проектных сетей. Получившееся количество округляется вверх до ближайшего целого, кратного двум.
...
* NVMe-диски необходимо разбивать на 2 раздела.
...
* {formula(V_D)} — объём диска, TiB. Для приведения объёма диска к TiB воспользуйтесь формулой {formula(V_D = \frac {u_D \ast 10^{12}}{2^{40}})}, где {formula(u_D)} — объём диска в ТБ.
...
* Минимальное количество ядер CPU на сервер:
   ...
   где {formula(Q_{OS})} — количество ядер под ОС (минимум 8 ядер).
